console.log(`%cFiles-card version: ${'0.1.6'} is installed.%c`, 'color: #EED202; font-weight: bold;background-color: black;', '');
      ...
      if (hass && hass.states[config.entity]) {
        ...
        if (feed !== undefined && Object.keys(feed).length > 0) {
              ...
              if (columns) {
                for (let column in columns) {
                  if (columns.hasOwnProperty(column) && feed[entry].hasOwnProperty(columns[column].field)) {
                    ...
                    ${tapAction || holdAction || doubleTapAction ? `data-filescardID='${JSON.stringify(feed[entry])}'` : ''}
          ...
          cells.forEach(cell => {
            ...
            const filescardID = cell.getAttribute('data-filescardID');

            if (tapAction) {
              cell.addEventListener('click', () => {
                this.handleAction(hass, JSON.parse(tapAction), filescardID);
              });
            }
            ...
            if (holdAction) {
              ...
              cell.addEventListener('touchstart', () => {
                timer = setTimeout(() => {
                  this.handleAction(hass, JSON.parse(holdAction), filescardID);
                }, 500);
              });
            ...
            if (doubleTapAction) {
              cell.addEventListener('dblclick', () => {
                this.handleAction(hass, JSON.parse(doubleTapAction), filescardID);
              });
            }
          });
        ...
        this.style.display = 'none';
                        ...
                        handleAction = (hass, action, filescardID) => {
                          if (action && action.action) {
                            // Replace placeholders in action data
                            const row = JSON.parse(filescardID);
                                  ...
                                  if (typeof obj[key] === 'string') {
                                      ...
                                      if (placeholders) {
                                          ...
                                          placeholders.forEach(placeholder => {
                                              ...
                                              // Find the row with matching filescardID
...
window.customCards.push({
  ...
  name: "Files Card",
  preview: false,
  description: "The Files Card generate a grid with data from sensor, json, or the folder integration that provides data as a list of attributes."
});
